...
#
# Method that runs the alpha-beta pruning algorithm and returns
# the move and score of each call.
#

def alphaBetaPruning(gameState, depth, player, opponent, alpha, beta):
    availableMoves = BOARD_SIZE_X
    for i in range(0, BOARD_SIZE_X):
        if gameState[0][i] != 0:
            availableMoves -= 1
    ...
    for i in range(0, BOARD_SIZE_X):
        ...
        # Recursive minimax call, with reduced depth
        move, score = alphaBetaPruning(gameState, depth - 1, opponent, player, alpha, beta)
...
def bestMove(gameState, player, opponent):
    ...
    global algo
    if(algo == 1):
      move, score = alphaBetaPruning(gameState, SEARCH_DEPTH, player, opponent, -math.inf, math.inf)
    else:
...
def playGame():
    ...
    print("=========================\n")
    algo = int(input("Enter the 1 if you want to execute Alpha-beta pruning, 0 to execute minimax"))
    ...
    while True:
        ...
        if algo == 1:
          print('algorithm: Alpha-beta pruning')
        else:
